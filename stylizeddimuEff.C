...
TCanvas* efficiencyPlots( int iPeriod, int iPos );
...
void dimuEff(
	...
	extraText  = "Performance"; // default extra text is "Preliminary"
	lumi_sqrtS = "5.02 TeV"; // used with iPeriod = 0, e.g. for simulation-only plots (default is an empty string)
	// lumi_sqrtS = "";
	...
	float           CenBin[nCenBin] = { 5, 15, 30, 50, 70, 90, 110, 130, 170 };
	float    	CenBinEdges[nCenBin + 1] = { 0, 10, 20, 40, 60, 80, 100, 120, 140, 200 };

	float           IntBin[1] = { 200 };
	float		IntBinEdges[2] = { 0, 200 };
...
EffCent->GetXaxis()->SetRangeUser(0.0, 200.0);
...
EffInt->GetXaxis()->SetRangeUser(0.0,200);
...
c1->cd();
EffCent->SetFillColor(0);
EffCent->SetBorderMode(0);
EffCent->SetFrameFillStyle(0);
EffCent->SetFrameBorderMode(0);
EffCent->SetLeftMargin( L/W );
EffCent->SetRightMargin( R/W );
EffCent->SetTopMargin( T/H );
EffCent->SetBottomMargin( B/H );
EffCent->SetTickx(0);
EffCent->SetTicky(0);
...
EffCent->GetYaxis()->SetTitle(Form("Efficiency[#varUpsilon(%dS)]_{%s}",oniaMode, isPbPb ? "PbPb" : "PP"));
EffCent->GetXaxis()->SetTitle(Form("%s",isPbPb ? "Centrality" : "Integrated Bin"));
EffCent->GetXaxis()->SetNdivisions(6,5,0);
EffCent->GetYaxis()->SetNdivisions(6,5,0);
